...
namespace BusinessLayer.Repository.Implementation
{
    public class AdminProfile :IAdminProfile
    {
        ...
        {
            var aspid = db.AspNetUsers.Where(a => a.Email == email).FirstOrDefault().Id;

            int id = db.Admins.Where(a=>a.AspNetUserId == aspid).FirstOrDefault().AdminId;
            return id;
        }
        public ProfileData AdminProfileDetails(int id)
        {
