...
{
    public class ProviderSchedule : IProviderSchedule
    {
        public HallodocContext _context;

        public ProviderSchedule(HallodocContext context) 
        {
            _context = context;
        ...
        public shift GetShiftDetails(int shiftid)
        {
            ...
            var shiftdetail = _context.ShiftDetails.Where(a => a.ShiftDetailId == shiftid).Include(a => a.Shift).ThenInclude(a => a.Physician).FirstOrDefault();

            shift obj = new shift()
            {
                shiftid = shiftdetail.ShiftDetailId,
                regionid = (int)shiftdetail.RegionId,
                regionName = regions.Where(a => a.RegionId == shiftdetail.RegionId).FirstOrDefault().Name,
                phyid = shiftdetail.Shift.PhysicianId,
                phyname = shiftdetail.Shift.Physician.FirstName + " " + shiftdetail.Shift.Physician.LastName,
        ...
        public void ReturnShift(int id)
        {
            var shiftdetail = _context.ShiftDetails.Where(a => a.ShiftDetailId == id).FirstOrDefault();
        ...
        public void EditShift(int shiftId, DateTime Startdate, TimeOnly Starttime, TimeOnly Endtime)
        {
            var shift = _context.ShiftDetails.Where(a => a.ShiftDetailId == shiftId).FirstOrDefault();
